...
struct class_rw_t {
    ...
    //成员变量信息则是存储在class_ro_t内部中的，我们来到class_ro_t
//    内查看
    const class_ro_t *ro;
...
/*
 isa指针中存在objc_object 的结构提当中，这是一个c++结构体，在c上做了扩展
 */
struct objc_class : objc_object {
   ...
   //用于获取具体的类信息
    ...
    //存储着方法列表，属性列表，协议列表，从下面可以看出class_rw_t 是通过bits调用data方法得来的，
//    进入方法内部可以查看到具体实现
    class_rw_t *data() {
        return bits.data();
    }
    ...
    uint32_t unalignedInstanceSize() {
        assert(isRealized());
        //不难看出就是从cls的ro中获得instanceSize然后将它对齐
...
//        从 cls 的 ro 中获得 instanceSize 然后将它对齐，并加上 extraBytes，并加上 extraBytes
        size_t size = alignedInstanceSize() + extraBytes;
        // CF requires all objects be at least 16 bytes.
//        需要对象至少要有 16 字节，小于 16 的统统返回 16
        if (size < 16) size = 16;
        return size;
    }
