...
//    对象真正的引用计数存储区。所以咱们说的引用计数加一或者减一，实际上是对整个unsigned long加四或者减四,因为真正的计数是从2^2位开始的。
...
/*
 SideTables 中包含SideTable，SideTable包含 ，
 //保证原子操作的自旋锁
 spinlock_t slock;
 //引用计数的hash表
 RefcountMap refcnts;
 //weak 引用计数的hash表
 weak_table_t weak_table;------>//保存了所有指向指定对象的weak指针
...
storeWeak(id *location, objc_object *newObj)
{
 ...
 retry:
    if (haveOld) {
        //如果存在旧值，那么就更改指针，oldTable存储oldObj为索引的值的地址
        oldObj = *location;
    ...
    //location 应该与oldObj保持一致，如果不同，说明当前的location已经处理过oldObj,可是又被其他线程修改了
 ...
 *weak 原理讲解
 ...
 可以理解成objc_storeWeak(value, key)，并且当key变nil，将value置nil。
...
{
    //newObj 是初始化的对象 key
    //location 是weak变量的地址,作为value
    //判断对象实例是否有效，如果对象无效，那么直接将location指针释放
//    也就是将hash表中的value直接设置成nil
    if (!newObj) {
        *location = nil;
        return nil;
    }

    //这里传递了三个bool数值,使用了template进行常量参数传递是为了优化性能
...
//        str1为第一个要比较的字符串，str2为第二个要比较的字符串，n为指定的str1与str2的比较的字符数。
...
class AutoreleasePoolPage 
{
    ...
    // never uses them.
#   define EMPTY_POOL_PLACEHOLDER ((id*)1) //被存放在本地线程存储中当一个池入栈并且没有存放  可以节省内存
...
#endif
//    每个 AutoreleasepoolPage 对象的大小都是 4096 个字节。
     //对象数量
    static size_t const COUNT = SIZE / sizeof(id);

    magic_t const magic;//校验完整性
    id *next;//指向最新添加的 autoreleased 对象的下一个位置，初始化时指向 begin() ；另外，当 next == begin() 时，表示 AutoreleasePoolPage 为空；当 next == end() 时，表示 AutoreleasePoolPage 已满。
    pthread_t const thread; //指向当前线程
    AutoreleasePoolPage * const parent; //指向父结点，第一个结点的 parent 值为 nil ；
    AutoreleasePoolPage *child;//指向子结点，最后一个结点的 child 值为 nil ；
    uint32_t const depth;//表深度，从 0 开始，往后递增 1
    ...
    //设置当前内存（可读可写） mprotect(this, SIZE, PROT_READ | PROT_WRITE);
    inline void protect() {
#if PROTECT_AUTORELEASEPOOL
...
//杀死方法，里面的判断逻辑要是不清楚的话，可以对照这两张图http://blog.leichunfeng.com/images/AutoreleasePoolPage1.png   http://blog.leichunfeng.com/images/AutoreleasePoolPage2.png
    void kill() 
    {
        // Not recursive: we don't want to blow out the stack 
        // if a thread accumulates a stupendous amount of garbage
...
//参考文档(http://www.dlbaoma.com/subject/cwwqrttx.html)
...
/*强制内联，修饰的函数在被调用的时候不会被编译成函数调用，而是直接
扩展到调用函数体内,当我们用inline__attribute__((always_inline))
 修饰一个函数的时候，编译器会将我们的代码编译，在调用的地方将我们的函数，插入到调用的地方
 举个例子说明一下：
 __inline__ attribute((always_inline)) void a(){
 print("a");
 }
 void b()
 {
 ...
 内联函数在编译器最终生成的代码中是没有定义的，这个函数是不存在的，内联函数也就没有
 普通函数调用时的额外开销压栈，跳转，返回)，内联函数是一种特殊的函数
 具有普通函数的特征
...
//如果是alloc callAlloc(cls, false/*checkNil*/, true/*allocWithZone*/);
...
//    接下来从 #if __OBJC2__ 开始至 #endif，都是在 Objective-C 2.0 下才会有的代码
#if __OBJC2__
//    这里判断一个类是否有自定义的 +allocWithZone: 实现，有的话，就不能走这里面的默认实现了。
    if (fastpath(!cls->ISA()->hasCustomAWZ())) {
        // No alloc/allocWithZone implementation. Go straight to the allocator.
        // fixme store hasCustomAWZ in the non-meta class and 
        // add it to canAllocFast's summary
        
        //目前，canAllocFast目前是永远返回NO，所以这里直接看else代码
...
//初始化
//从源码中可以看出，init其实什么也没有做，直接就返回了对象obj
