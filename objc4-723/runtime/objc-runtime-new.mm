...
//_class_createInstanceFromZone，其第三个参数是 zone，用来表示 NSZone，但是 NSZone 已经是个没有使用的东西，所以这里直接传入了 nil
                              ...
                              size_t *outAllocatedSize = nil)
{
    ...
    // Read class's info bits all at once for performance
    bool hasCxxCtor = cls->hasCxxCtor();//是否有C++类搞糟函数
    bool hasCxxDtor = cls->hasCxxDtor();//析构函数
//    是对 isa 的类型的区分，如果一个类的实例不能使用 isa_t 类型的 isa 的话，fast 就为 false
    bool fast = cls->canAllocNonpointer();
...
//        因为 zone 肯定是没有的，NSZone 已经弃用了，fast 几乎也都是真，在这个分支里面，obj 首先使用大家都熟悉的 C 标准库这套内存申请函数中的 calloc 申请了一个 size 大小的空间
