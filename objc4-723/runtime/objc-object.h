...
{ 
    //断言对象的指针是不是TaggedPointer
    assert(!isTaggedPointer());
    
    //判断isa类型
    
//    如果 isa 不是 nonpointer，也就是单纯的指针（Class 类型），则 isa 就直接被赋值为 cls
    if (!nonpointer) {
        isa.cls = cls;
    } else {
        assert(!DisableNonpointerIsa);
        assert(!cls->instancesRequireRawIsa());
//如果 isa 是 nonpointer，也就是 isa_t 类型的话，则先初始化一个所有位为 0 的指针 isa_t newisa(0)。
        isa_t newisa(0);
...
//retain 实现，默认传入两个false，false
ALWAYS_INLINE id 
objc_object::rootRetain(bool tryRetain, bool handleOverflow)
{
    if (isTaggedPointer()) return (id)this;

    bool sideTableLocked = false; //用来表示side table是否锁上了
    bool transcribeToSideTable = false; //用来表示是否需要将isa中
//    的引用计数转移到side table里去

    isa_t oldisa; //isa本来的h值
    isa_t newisa; //isa新的值，（增加了引用计数后的值）

    do {
        transcribeToSideTable = false;
        /*
         循环里首先将 transcribeToSideTable 赋值为 false，oldisa 和 newisa 赋值为 isa.bits 的值（LoadExclusive 的作用是让读取操作原子化，根据 CPU 不同实现不同，比如在 x86-64 上就是单纯的直接返回值，而在 arm64 上则使用了 ldxr 指令）
         */
        oldisa = LoadExclusive(&isa.bits);
        newisa = oldisa;
        //首先会检查 isa 是不是 non-pointer,如果不是 non-pointer，就进入 sidetable_retain 这个过程
        if (slowpath(!newisa.nonpointer)) {
            ClearExclusive(&isa.bits);
