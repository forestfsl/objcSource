...
struct weak_entry_t {
    DisguisedPtr<objc_object> referent; //指针(被指对象的地址。前面循环遍历查找的时候就是判断目标地址是否和他相等。)
    union {
        struct {
            weak_referrer_t *referrers; //可变数组(变数组,里面保存着所有指向这个对象的弱引用的地址。当这个对象被释放的时候，referrers里的所有指针都会被设置成nil。)
            ...
            uintptr_t        max_hash_displacement;//元素上限阀值。
        };
        struct {
            // out_of_line_ness field is low bits of inline_referrers[1]
            weak_referrer_t  inline_referrers[WEAK_INLINE_COUNT]; //数组(只有4个元素的数组，默认情况下用它来存储弱引用的指针。当大于4个的时候使用referrers来存储指针。)
        };
    };
 ...
 */
/*weak表是一个弱引用表，实现为一个weak_table_t结构体，
*存储了某个对象相关的的所有的弱引用信息
 */
...
struct weak_table_t {
    ...
    //存储空间
    size_t    num_entries; //num_entries是用来维护保证数组始终有一个合适的size。比如数组中元素的数量超过3/4的时候将数组的大小乘以2。
    //参与判断引用计数辅助量
    uintptr_t mask;
    //hash key 最大偏移值
